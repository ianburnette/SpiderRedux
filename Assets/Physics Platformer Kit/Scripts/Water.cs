...
public class Water : MonoBehaviour 
{
    ...
    public float surfaceModifier = 2f;

    [SerializeField]
    private float jumpDepth, deepDepth;
	...
	{
        if (movementScript == null && other.transform.tag == "Player")
            movementScript = other.GetComponent<PlayerMove>();
		...
		if(rigid)
		{
            ...
            //if we are near the surface, add less force, this prevents objects from "jittering" up and down on the surface
                ...
                rigid.AddForce(force * (depth * surfaceModifier), ForceMode.Force);
            if (depth > jumpDepth && depth < deepDepth && other.transform.tag == "Player")
                movementScript.InWater = true;
            else if (depth < jumpDepth && other.transform.tag == "Player")
                movementScript.InWater = false;
