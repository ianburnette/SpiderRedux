...
public class CustomPlayerMove : MonoBehaviour
{
    //setup
    ...
    void Update()
    {
       ...
       // if (cameraScript.CamState != ThirdPersonCamera.CamStates.FirstPerson)
       // {
    ...
    public void StickToWorldSpace(Transform root, Transform camera, ref Vector3 directionOut, ref float speedOut)
    {
       ...
       // if (cameraScript.CamState == ThirdPersonCamera.CamStates.Target)
       //     rotateSpeed = 0f;
       // else
       // {
    ...
    void FixedUpdate()
    {
        ...
        float accel = (grounded) ? accelCurve.Evaluate(direction.magnitude) * speedMultiplier : accelCurve.Evaluate(direction.magnitude) * airAccel;
        Vector3 prevVector = slopeMovementVector;
        //ledgeHandling.LedgeMovementFilter(ref slopeMovementVector);
        //if (prevVector !=)
